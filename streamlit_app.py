...
def main():
    ...
    with tab3:
        ...
        - Players who don't sign: the model does not include any predictions about which players may not sign, and therefore also fails to take into account the hypothetical 
        value of compensatory picks when this occurs.
        - More rounds: the model only takes into account rounds 1-4, as a result of limited player data. This means that the model is constrained in how creative its strategies
        can be. For example, drafting players and signing them well above slot value, which is a phenomenon that mostly occurs later in the draft, is not a strategy utilized 
        by this model.
        - More robust iterations: For example, if a very valuable player is available the second round about 40% of the time, it may be worth it to save the necessary money in 
